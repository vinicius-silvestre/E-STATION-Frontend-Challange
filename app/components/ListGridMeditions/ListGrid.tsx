...
function ListItem() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3001/data?_page=1&_limit=10`)
            .then((res) => {
                setAgent(res.data);
                
                // console.log(agent, 'teste');
            })
            setTimeout( async () =>{
        ...
        <div className="pt-10 pl-10 pr-10">
            ...
            <div className="bg-white p-8 shadow-md rounded-md space-y-2 grid-rows-1	 ">
                ...
                <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between pt-5 pr-5 pl-5">
                    <div>
                        <div className=' text-sm text-slate-500'>Exibindo 10 de {agent.length} items</div>
                    </div>
                    <div>
                        <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                            <a href="#" className="relative inline-flex items-center   ">
                                <a href="#" className="inline-flex items-center px-4 py-2 mr-3 text-sm font-medium text-blue-600 bg-white border border-blue-600 rounded-lg hover:bg-slate-100 hover:text-blue-400 dark:bg-gray-800 dark:border-blue-400 dark:text-gray-400 dark:hover:bg-gray-200 dark:hover:text-white">
                                   Anterior
                                </a>
                                <a href="#" className="inline-flex items-center px-4 py-2 mr-3 text-sm font-medium text-blue-600 bg-white border border-blue-600 rounded-lg hover:bg-slate-100 hover:text-blue-400 dark:bg-gray-800 dark:border-blue-400 dark:text-gray-400 dark:hover:bg-gray-200 dark:hover:text-white">
                                    Próximo
